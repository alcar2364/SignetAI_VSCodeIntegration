...
- A workspace-scoped `PreCompact` hook sends lightweight metadata to Signet and records local state
  without uploading transcript content.
...
- A workspace-scoped `Stop` hook performs the final session-end upload and includes
  transcript content when it is available.
...
- [.github/agents/signet-codex-vscode.agent.md](../../../agents/signet-codex-vscode.agent.md)
...
- [.github/hooks/signet-session-start.ps1](../../../hooks/signet-session-start.ps1)
...
- [.github/hooks/signet-user-prompt-submit.ps1](../../../hooks/signet-user-prompt-submit.ps1)

### 5. Implement pre-compaction signaling

The `PreCompact` hook should:

- Read `transcript_path` and session metadata from VS Code hook input.
- Notify Signet that compaction is happening using harness, session metadata,
  and transcript path.
- Keep the call best-effort so compaction is never blocked on Signet.
- Record lightweight local state if the Stop hook needs per-session compaction
  history.

See:

- [.github/hooks/signet-pre-compaction.json](../../../../.github/hooks/signet-pre-compaction.json)
- [.github/hooks/signet-pre-compaction.ps1](../../../hooks/signet-pre-compaction.ps1)
- [.github/hooks/signet-transcript-state.ps1](../../../hooks/signet-transcript-state.ps1)
...
- Forward transcript path as-is and attach transcript content when it can be
  read safely.
- Clear local transcript state only after Signet accepts the final upload.
...
prompt refresh, pre-compaction signaling, and session-end reporting can run for
all VS Code agent sessions.

See:

- [.github/hooks/signet-session-end.json](../../../../.github/hooks/signet-session-end.json)
- [.github/hooks/signet-session-end.ps1](../../../hooks/signet-session-end.ps1)
- [.github/hooks/signet-transcript-state.ps1](../../../hooks/signet-transcript-state.ps1)
...
- Per-session prompt-refresh markers
- Transcript state files for pre-compaction history and final cleanup
...
2. Confirm session start leaves the live-context file in an inactive
   placeholder state.
3. Confirm `PreCompact` sends only lightweight metadata and does not block
   compaction when Signet is unavailable.
4. Confirm `PreCompact` records local transcript state for the session.
...
7. Confirm `Stop` forwards transcript content when available and clears local
...
- Do not terminate the live Signet session during pre-compaction.
- Prefer metadata-only signaling unless transcript extraction at pre-compaction
  is a hard requirement.
- Keep any local state minimal and safe to discard.
...
- The harness is debuggable from the agent file, hook scripts, workspace
  instructions, and generated artifacts without hidden coupling.

## Current Repo Gaps To Revisit

- Whether [.github/agents/signet-codex-vscode.agent.md](../../../agents/signet-codex-vscode.agent.md) should be renamed to reflect the shared `vscode-custom-agent` harness more clearly.
- Whether generated live-context and debug payload files should be added to a
  `.gitignore` policy or intentionally committed as diagnostics.
- Whether this workflow should also be exposed through a prompt file for faster reuse.
